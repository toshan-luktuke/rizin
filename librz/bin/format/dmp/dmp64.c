...
static bool rz_bin_dmp64_init_triage_drivers(struct rz_bin_dmp64_obj_t *obj) {
	...
	for (i = 0; i < obj->triage64_header->DriverCount; i++) {
		...
		ut16 *file = calloc(str.count + 1, sizeof(ut16));
		...
		int idx = 0, idx_utf8 = 0;
		while (idx < (str.count * sizeof(ut16)) && rz_read_le16(file + idx)) {
			RzRune ch;
			idx += rz_utf16le_decode((ut8 *)(file + idx), (str.count - idx) * sizeof(ut16), &ch);
			idx_utf8 += rz_utf8_encode(file_utf8 + idx_utf8, ch);
		}
