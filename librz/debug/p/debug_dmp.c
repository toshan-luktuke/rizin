...
static bool rz_debug_dmp_init(RzDebug *dbg, void **user) {
	...
	const ut64 state_offset = rz_type_db_struct_member_offset(dbg->analysis->typedb, "_KPRCB", "ProcessorState");
	ctx->kprcb_context_offset = state_offset + rz_type_db_struct_member_offset(dbg->analysis->typedb, "_KPROCESSOR_STATE", "ContextFrame");
