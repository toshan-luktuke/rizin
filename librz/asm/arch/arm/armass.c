...
static ut64 cqcheck(char **input) {
	...
	for (i = 0; conds[i]; i++) {
		if (rz_str_startswith(*input, conds[i])) {
			res |= C_MATCH_BIT;
			res |= i << 2;
			*input += 2;
			offset += 2;
	...
	if (rz_str_startswith(*input, ".n")) {
		res |= DOTN_BIT;
		*input += 2;
		offset += 2;
	} else if (rz_str_startswith(*input, ".w")) {
		res |= DOTW_BIT;
		*input += 2;
		offset += 2;
	}
...
static ut64 opmask(char *input, char *opcode, ut64 allowed_mask) {
	...
	if (rz_str_startswith(input, opcode)) {
		input += strlen(opcode);
		res |= M_BIT;
		res |= cqcheck(&input);
		...
		if ((rz_str_startswith(input, "wb")) && (WB_BIT & allowed_mask)) {
			res |= WB_BIT;
			input += 2;
		}
		if ((rz_str_startswith(input, "wt")) && (WT_BIT & allowed_mask)) {
			res |= WT_BIT;
			input += 2;
		}
		res |= cqcheck(&input);
		if ((rz_str_startswith(input, "db")) && (DB_BIT & allowed_mask)) {
			res |= DB_BIT;
			input += 2;
		}
		if ((rz_str_startswith(input, "ea")) && (EA_BIT & allowed_mask)) {
			res |= EA_BIT;
			input += 2;
		}
		if ((rz_str_startswith(input, "ia")) && (IA_BIT & allowed_mask)) {
			res |= IA_BIT;
			input += 2;
		}
		...
		res |= cqcheck(&input);
		if ((rz_str_startswith(input, "bb")) && (BB_BIT & allowed_mask)) {
			res |= BB_BIT;
			input += 2;
		}
		if ((rz_str_startswith(input, "tt")) && (TT_BIT & allowed_mask)) {
			res |= TT_BIT;
			input += 2;
		}
		if ((rz_str_startswith(input, "bt")) && (BT_BIT & allowed_mask)) {
			res |= BT_BIT;
			input += 2;
		}
		...
		if ((*input == 'b') && (B_BIT & allowed_mask)) {
			res |= B_BIT;
			input++;
		} else if ((*input == 'h') && (H_BIT & allowed_mask)) {
			res |= H_BIT;
			input++;
		} else if ((*input == 'd') && (D_BIT & allowed_mask)) {
			res |= D_BIT;
			input++;
		}
		...
		if ((rz_str_startswith(input, "16")) && (SIXTEEN_BIT & allowed_mask)) {
			res |= SIXTEEN_BIT;
			input += 2;
		}
		res |= cqcheck(&input);
		...
		res |= cqcheck(&input);
		if ((rz_str_startswith(input, "id")) && (ID_BIT & allowed_mask)) {
			res |= ID_BIT;
			input += 2;
		}
		if ((rz_str_startswith(input, "ie")) && (IE_BIT & allowed_mask)) {
			res |= IE_BIT;
			input += 2;
		}
		res |= cqcheck(&input);
		if ((rz_str_startswith(input, "sh")) && (SH_BIT & allowed_mask)) {
			res |= SH_BIT;
			input += 2;
		}
		res |= cqcheck(&input);
...
static ut64 getnumbang(const char *str) {
	...
	if (!str || !*str || !rz_str_endswith(str, "!")) {
		err = true;
		return 0;
	}
	char *temp = rz_str_ndup(str, strlen(str) - 1);
	if (!temp) {
		return -1;
	}
	err = false;
...
static st32 getregmemstartend(const char *input) {
	st32 res;
	if (!input || (strlen(input) < 2) || (*input != '[') || !rz_str_endswith(input, "]")) {
		return -1;
	}
	input++;
	char *temp = rz_str_ndup(input, strlen(input) - 1);
	if (!temp) {
		return -1;
	}
	res = getreg(temp);
...
static st32 getregmemend(const char *input) {
	st32 res;
	if (!input || !*input || !rz_str_endswith(input, "]")) {
		return -1;
	}
...
static st32 getnummemendbang(const char *input) {
	st32 res;
	err = false;
	if (!input || (strlen(input) < 2) || (input[strlen(input) - 2] != ']' || !rz_str_endswith(input, "!"))) {
		err = true;
		return 0;
	}
	char *temp = rz_str_ndup(input, strlen(input) - 2);
	if (!temp) {
		err = true;
		return 0;
	}
...
static ut8 interpret_msrbank(char *str, ut8 *spsr) {
	...
	int res = 0;
	int i, j;
	if (rz_str_startswith(str, "spsr_")) {
		*spsr = 1;
	} else {
		*spsr = 0;
	}

	if (rz_str_startswith(str, "apsr_")) {
		if (!(strcmp(str + 5, "g"))) {
			return 0x4;
		}
	...
	if (rz_str_startswith(str, "cpsr_") || rz_str_startswith(str, "spsr_")) {
		for (i = 0; str[5 + i]; i++) {
			for (j = 0; fields[j]; j++) {
				if (str[5 + i] == fields[j]) {
					break;
...
static ut32 getshift(const char *str) {
	...
	if (rz_str_casecmp(type, shifts[5])) {
		space = strchr(type, ' ');
		if (!space) {
			return 0;
		}
		*space = 0;
		...
		for (i = 0; shifts[i]; i++) {
			if (!rz_str_casecmp(type, shifts[i])) {
				shift = 1;
				break;
			}
		}
...
static int thumb_assemble(ArmOpcode *ao, ut64 off, const char *str) {
	...
	if (!strcmpnull(ao->op, "udf")) {
		...
		return 2;
	} else if ((m = opmask(ao->op, "add", S_BIT | W_BIT))) {
	...
	} else if ((m = opmask(ao->op, "adc", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_CONST: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "and", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			ao->o = 0x0040;
	...
	} else if ((m = opmask(ao->op, "asr", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG_CONST: {
			ut8 reg1 = getreg(ao->a[0]);
	...
	} else if ((m = opmask(ao->op, "bic", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			ao->o = 0x8043;
	...
	} else if ((m = opmask(ao->op, "cdp", TWO_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
	...
	} else if ((m = opmask(ao->op, "cps", ID_BIT | IE_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_OTHER: {
	...
	} else if ((m = opmask(ao->op, "eor", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_CONST:
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "ldc", TWO_BIT | L_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
	...
	} else if ((m = opmask(ao->op, "ldm", DB_BIT | EA_BIT | IA_BIT | FD_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
	...
	} else if ((m = opmask(ao->op, "ldr", B_BIT | H_BIT | D_BIT | T_BIT | S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
	...
	} else if ((m = opmask(ao->op, "ldrex", B_BIT | H_BIT | D_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		ut32 ldrsel = m & (B_BIT | H_BIT | D_BIT);
		switch (argt) {
		case THUMB_REG_BRACKREGBRACK: {
	...
	} else if ((m = opmask(ao->op, "lsl", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG_CONST: {
			ut8 reg1 = getreg(ao->a[0]);
	...
	} else if ((m = opmask(ao->op, "mcr", RZ_BIT | TWO_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
	...
	} else if ((m = opmask(ao->op, "mov", S_BIT | W_BIT | T_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_CONST: {
			ut32 reg1 = getreg(ao->a[0]);
	...
	} else if ((m = opmask(ao->op, "mrc", TWO_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_COPROC_CONST_REG_COREG_COREG: {
			ao->a[5] = "0";
	...
	} else if ((m = opmask(ao->op, "mrrc", TWO_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_COPROC_CONST_REG_REG_COREG: {
			ut32 coproc = getcoproc(ao->a[0]);
	...
	} else if ((m = opmask(ao->op, "mvn", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_CONST: {
			ut8 reg1 = getreg(ao->a[0]);
	...
	} else if ((m = opmask(ao->op, "orn", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_CONST: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "orr", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_CONST: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "pkh", BT_BIT | TB_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "qadd", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "qdadd", 0))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "qdsub", 0))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "qsub", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "rev", SIXTEEN_BIT | SH_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			if (m & SIXTEEN_BIT) {
	...
	} else if ((m = opmask(ao->op, "rfe", IA_BIT | FD_BIT | DB_BIT | EA_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		ut32 wb = 0;
		switch (argt) {
		case THUMB_REGBANG: {
	...
	} else if ((m = opmask(ao->op, "ror", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG_CONST: {
			ut8 reg1 = getreg(ao->a[0]);
	...
	} else if ((m = opmask(ao->op, "rrx", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			ut8 reg1 = getreg(ao->a[0]);
	...
	} else if ((m = opmask(ao->op, "rsb", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_CONST: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "sadd", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "sbc", S_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			ao->o = 0x8041;
	...
	} else if ((m = opmask(ao->op, "shadd", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "shasx", 0))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "shsax", 0))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "shsub", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "smla", BB_BIT | BT_BIT | TB_BIT | TT_BIT | WB_BIT | WT_BIT | L_BIT | D_BIT | X_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG_REG_REG: {
			ut8 reg1 = getreg(ao->a[0]);
	...
	} else if ((m = opmask(ao->op, "smlsd", X_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG_REG_REG: {
			ut8 reg4 = getreg(ao->a[3]);
	...
	} else if ((m = opmask(ao->op, "smlsld", X_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG_REG_REG: {
			ut8 reg1 = getreg(ao->a[0]);
	...
	} else if ((m = opmask(ao->op, "smmla", RZ_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG_REG_REG: {
			ut8 reg4 = getreg(ao->a[3]);
	...
	} else if ((m = opmask(ao->op, "smmls", RZ_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG_REG_REG: {
			ut8 reg4 = getreg(ao->a[3]);
	...
	} else if ((m = opmask(ao->op, "smmul", RZ_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "smuad", X_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "smul", BB_BIT | BT_BIT | TB_BIT | TT_BIT | WB_BIT | WT_BIT | L_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "smusd", X_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "srs", DB_BIT | FD_BIT | IA_BIT | EA_BIT))) {
		ut32 w = 0;
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
	...
	} else if ((m = opmask(ao->op, "ssat", SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
	...
	} else if ((m = opmask(ao->op, "ssub", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
		...
		case THUMB_REG_BRACKREG_CONSTBRACK: {
			...
			if (m & T_BIT) {
				...
				if (strsel == 0) {
					ao->o = 0x40f8000e;
				} else if (strsel == H_BIT) {
					ao->o = 0x20f8000e;
				} else if (strsel == B_BIT) {
					ao->o = 0x00f8000e;
				} else {
					return -1;
				}
	...
	} else if ((m = opmask(ao->op, "strex", B_BIT | D_BIT | H_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		ut32 strsel = m & (B_BIT | H_BIT | D_BIT);
		switch (argt) {
		case THUMB_REG_REG_BRACKREGBRACK: {
	...
	} else if ((m = opmask(ao->op, "sub", S_BIT | W_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_CONST: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "sxta", B_BIT | H_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "sxt", B_BIT | H_BIT | SIXTEEN_BIT))) {
		ut64 sufsel = m & (B_BIT | H_BIT | SIXTEEN_BIT);
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
	...
	} else if ((m = opmask(ao->op, "tb", B_BIT | H_BIT))) {
		ut64 argt = thumb_selector(ao->a);
	...
	} else if ((m = opmask(ao->op, "uadd", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "uhadd", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
	...
	} else if ((m = opmask(ao->op, "uhasx", 0))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "uhsax", 0))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "uhsub", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		ut64 sufsel = m & (EIGHT_BIT | SIXTEEN_BIT);
		switch (argt) {
		case THUMB_REG_REG: {
	...
	} else if ((m = opmask(ao->op, "uqadd", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		ut64 sufsel = m & (EIGHT_BIT | SIXTEEN_BIT);
		switch (argt) {
		case THUMB_REG_REG: {
	...
	} else if ((m = opmask(ao->op, "uqasx", 0))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "uqsax", 0))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "uqsub", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		ut64 sufsel = m & (EIGHT_BIT | SIXTEEN_BIT);
		switch (argt) {
		case THUMB_REG_REG: {
	...
	} else if ((m = opmask(ao->op, "usad8", 0))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_REG: {
			std_opt_2(ao);
	...
	} else if ((m = opmask(ao->op, "usat", SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_REG_CONST_REG: {
			if (m & SIXTEEN_BIT) {
	...
	} else if ((m = opmask(ao->op, "usub", EIGHT_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		ut64 sufsel = m & (EIGHT_BIT | SIXTEEN_BIT);
		switch (argt) {
		case THUMB_REG_REG: {
	...
	} else if ((m = opmask(ao->op, "uxta", B_BIT | H_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		ut64 sufsel = m & (B_BIT | H_BIT | SIXTEEN_BIT);
		switch (argt) {
		case THUMB_REG_REG: {
	...
	} else if ((m = opmask(ao->op, "uxt", B_BIT | H_BIT | SIXTEEN_BIT))) {
		ut64 argt = thumb_selector(ao->a);
		ut64 sufsel = m & (B_BIT | H_BIT | SIXTEEN_BIT);
		switch (argt) {
		case THUMB_REG_REG: {
	...
	} else if ((m = opmask(ao->op, "yield", 0))) {
		ut64 argt = thumb_selector(ao->a);
		switch (argt) {
		case THUMB_NONE: {
			if (m & DOTW_BIT) {
